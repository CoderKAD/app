...
CREATE TABLE users (
    ...
    CONSTRAINT chk_users_roles CHECK (roles IN ('ADMIN', 'CUSTOMER', 'KITCHEN', 'CASHIER', 'DELIVERY'))
);
...
CREATE TABLE menu_items (
    ...
    updated_at TIMESTAMP NOT NULL DEFAULT NOW(),
    category_id UUID,
    CONSTRAINT fk_menu_items_category FOREIGN KEY (category_id) REFERENCES categories_menu (id) ON DELETE SET NULL,
...
CREATE TABLE restaurant_tables (
    ...
    CONSTRAINT fk_restaurant_tables_user FOREIGN KEY (user_id) REFERENCES users (id) ON DELETE SET NULL,
...
CREATE TABLE orders (
    ...
    CONSTRAINT chk_orders_type_order CHECK (type_order IN ('DINE_IN', 'DELIVERY')),
    CONSTRAINT chk_orders_status CHECK (status IN ('PENDING', 'CONFIRMED', 'PREPARING', 'READY', 'OUT_FOR_DELIVERY', 'COMPLETED', 'CANCELLED')),
...
CREATE TABLE reservations (
    ...
    CONSTRAINT chk_reservations_status CHECK (status IN ('PENDING', 'CONFIRMED', 'CANCELLED', 'NO_SHOW', 'COMPLETED'))
);
...
CREATE TABLE reservation_tables (
    ...
    CONSTRAINT fk_reservation_tables_reservation FOREIGN KEY (reservation_id) REFERENCES reservations (id) ON DELETE CASCADE,
    CONSTRAINT fk_reservation_tables_table FOREIGN KEY (table_id) REFERENCES restaurant_tables (id) ON DELETE CASCADE
);
...
CREATE TABLE reservation_request (
    ...
    reservation_id UUID NOT NULL,
    user_id UUID,
    CONSTRAINT fk_reservation_request_reservation FOREIGN KEY (reservation_id) REFERENCES reservations (id) ON DELETE CASCADE,
    CONSTRAINT fk_reservation_request_user FOREIGN KEY (user_id) REFERENCES users (id) ON DELETE SET NULL,
    CONSTRAINT chk_reservation_request_status CHECK (status IN ('ACCEPTED', 'REFUSED', 'PENDING'))
);
